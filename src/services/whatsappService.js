...
async function resolveFinalUrl(url, timeoutMs = 15000) {
  try {
    ...
    if (isHttpUrl(response?.url)) return response.url;
  } catch (error) {
    // Share/short links that fail to resolve just fall back to the original URL.
  }
  return url;
}
...
class WhatsAppService {
  ...
  async handleBuiltInCommand(chatId, message, content, text) {
    ...
    if (command === '.facebook' || command === '!facebook' || command === '.fb' || command === '!fb') {
      await this.handleFacebookCommand(chatId, message, stripCommandPrefix(normalized));
      return true;
    }

    if (command === '.instagram' || command === '!instagram' || command === '.ig' || command === '!ig') {
      await this.handleInstagramCommand(chatId, message, stripCommandPrefix(normalized));
      return true;
    }

    if (command === '.igs' || command === '!igs') {
      await this.handleInstagramStickerCommand(chatId, message, stripCommandPrefix(normalized));
      return true;
    }

    if (command === '.removebg' || command === '!removebg' || command === '.rmbg' || command === '!rmbg' || command === '.nobg' || command === '!nobg') {
      await this.handleRemoveBgCommand(chatId, message, stripCommandPrefix(normalized), content);
      return true;
    }
  ...
  extractFacebookVideoUrl(data) {
    ...
    // siputzx format: { status: true, data: { downloads: [{ quality, type, url }] } }
    if (Array.isArray(data?.data?.downloads) && data.data.downloads.length) {
      const downloads = data.data.downloads;
      const hd = downloads.find((item) => /hd|720|1080/i.test(String(item?.quality || '')) && isHttpUrl(item?.url));
      const sd = downloads.find((item) => isHttpUrl(item?.url));
      if (hd?.url) return hd.url;
      if (sd?.url) return sd.url;
    }
  ...
  extractFacebookVideoTitle(data) {
    return (
      data?.data?.title
  ...
  async handleFacebookCommand(chatId, message, args) {
    ...
    const rawUrl = String(args || '').trim();
    if (!rawUrl || !/facebook\.com|fb\.watch/i.test(rawUrl)) {
      await this.sock.sendMessage(
        chatId,
    ...
    // Share/short links (fb.watch, /share/r/, /share/v/) need to be resolved to their
    // canonical URL first, otherwise the downloader API often fails to find the video.
    const url = await resolveFinalUrl(rawUrl, 15000);

    const providers = [
      () => fetchJsonWithTimeout(`https://api.siputzx.my.id/api/d/facebook?url=${encodeURIComponent(url)}`, 30000),
      () => fetchJsonWithTimeout(`https://api.siputzx.my.id/api/d/facebook?url=${encodeURIComponent(rawUrl)}`, 30000),
    ];
    ...
    for (const fetchProvider of providers) {
      try {
        ...
        const title = this.extractFacebookVideoTitle(data);
        await this.sock.sendMessage(
          chatId,
        ...
        lastError = error;
        console.error('[WA] .facebook provider failed:', error.message);
      }
    }

    console.error('[WA] .facebook error:', lastError?.message);
    await this.sock.sendMessage(
      chatId,
      { text: '❌ Gagal download video Facebook. Pastikan link betul dan video bersifat awam (public), atau cuba lagi sebentar.' },
      { quoted: message }
    );
  }
  ...
  async fetchInstagramMediaUrls(rawUrl) {
    // Share links (instagram.com/share/reel/... or instagr.am/...) need resolving to the
    // canonical /p/, /reel/ or /tv/ URL first, otherwise the downloader API can't parse it.
    const resolvedUrl = await resolveFinalUrl(rawUrl, 15000);
    const urlsToTry = [...new Set([resolvedUrl, rawUrl])];
    const endpoints = urlsToTry.map(
      (u) => `https://api.yupra.my.id/api/downloader/instagram?url=${encodeURIComponent(u)}`
    );
    ...
    for (const endpoint of endpoints) {
      for (let attempt = 1; attempt <= 2; attempt += 1) {
        try {
          const data = await fetchJsonWithTimeout(endpoint, 30000);
          if (/invalid client token/i.test(data?.message || '')) {
            throw new Error('Provider Instagram sedang down (token upstream tidak sah)');
          }
          const mediaUrls = this.extractInstagramMediaUrls(data);
          if (mediaUrls.length) return mediaUrls;
          throw new Error('Media tidak ditemukan dalam respons API');
        } catch (error) {
          lastError = error;
          console.log(`[WA] Instagram provider failed (attempt ${attempt}):`, error.message);
          if (/token upstream tidak sah/i.test(error.message)) break;
          if (attempt < 2) {
    ...
    throw lastError || new Error('Semua provider Instagram gagal');
  }
  ...
  async handleInstagramCommand(chatId, message, args) {
    ...
    try {
      const mediaUrls = await this.fetchInstagramMediaUrls(url);
      ...
      await this.sock.sendMessage(
        chatId,
        { text: this.buildInstagramFailureMessage(error) },
        { quoted: message }
      );
    }
  }

  resolveImageOnlyMedia(content) {
    const quoted = content?.extendedTextMessage?.contextInfo?.quotedMessage || null;
    const sources = [quoted, content];

    for (const source of sources) {
      if (source?.imageMessage) return source.imageMessage;
    }

    return null;
  }

  async uploadBufferToCatbox(buffer, fileName) {
  ...
  async handleRemoveBgCommand(chatId, message, args, content) {
    if (!this.sock) return;

    const usageText = '📸 Remove Background\n\nUsage:\n.removebg <image_url>\natau reply/kirim gambar dengan .removebg';
    const providedUrl = String(args || '').trim();

    if (providedUrl && !isHttpUrl(providedUrl)) {
      await this.sock.sendMessage(
        chatId,
    ...
    try {
      ...
      if (!imageUrl) {
        const imageMedia = this.resolveImageOnlyMedia(content);
        if (!imageMedia) {
          await this.sock.sendMessage(chatId, { text: usageText }, { quoted: message });
          return;
        }

        const stream = await downloadContentFromMessage(imageMedia, 'image');
        const buffer = await this.streamToBuffer(stream);
        imageUrl = await this.uploadBufferToCatbox(buffer, `removebg_${Date.now()}.jpg`);
      }
      ...
      await this.sock.sendMessage(
        chatId,
        { image: resultBuffer, caption: '✨ Background removed successfully!' },
        { quoted: message }
      );
    } catch (error) {
      console.error('[WA] .removebg error:', error.message);
      await this.sock.sendMessage(
        chatId,
        { text: '❌ Gagal remove background. Cuba link/gambar lain.' },
        { quoted: message }
      );
    }
  }

  async handleInstagramStickerCommand(chatId, message, args) {
    if (!this.sock) return;
    ...
    if (!url || !/instagram\.com|instagr\.am/i.test(url)) {
      await this.sock.sendMessage(
        chatId,
        { text: 'Usage: .igs <link-instagram>\nContoh: .igs https://www.instagram.com/reel/...' },
        { quoted: message }
      );
      return;
    }
    ...
    try {
      ...
      const mediaResponse = await fetchWithTimeout(mediaUrl, {}, 40000);
      if (!mediaResponse.ok) throw new Error(`HTTP ${mediaResponse.status}`);

      const buffer = Buffer.from(await mediaResponse.arrayBuffer());
      const stickerBuffer = await this.convertToWebp(buffer, isVideo ? 'video' : 'image');
      await this.sock.sendMessage(chatId, { sticker: stickerBuffer }, { quoted: message });
    } catch (error) {
      console.error('[WA] .igs error:', error.message);
      await this.sock.sendMessage(
        chatId,
        { text: this.buildInstagramFailureMessage(error) },
        { quoted: message }
  ...
  buildInstagramFailureMessage(error) {
    if (/token upstream tidak sah/i.test(error?.message || '')) {
      return '❌ Provider Instagram sedang down di pihak penyedia API (bukan isu bot). Cuba lagi sebentar nanti.';
    }
    return '❌ Gagal download media Instagram. Pastikan link betul dan post bersifat awam (public), atau cuba lagi sebentar.';
  }
